...
/**
 * versionLabel → 이 node가 그 버전으로 **처음** 나갔던 release 하나(설계서 05장 §7.3).
 *
 ...
 */
export function releaseBadgeMap(releases: ReleaseDto[], nodeId: string): Map<string, ReleaseBadge> {
  const map = new Map<string, ReleaseBadge>();
  const sorted = [...releases].sort((a, b) => a.seq - b.seq);
  for (const r of sorted) {
    const item = r.items.find((i) => i.nodeId === nodeId);
