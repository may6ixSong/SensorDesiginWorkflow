...
/**
 * 상세 패널 A(왼쪽) 영역의 "내용" 카드 한 장 — 지금 고른 버전의 문서면.
 * 스크롤·가운데 정렬은 부모(DeliverableDialog의 A 컬럼)가 맡는다 — 이 컴포넌트는
 * 카드 자체만 그려서, 그 아래 버전 기록 폼과 같은 컬럼에 나란히 놓일 수 있게 한다.
 *
  ...
  return (
    ...
    >
      ...
      {/* 문서면 아래 띠 — 이 산출물 자체의 맥락(버전이 아니라 산출물에 붙는 값들).
          Artifact key는 내부 매핑용 값이라 여기 보여줄 필요가 없다(사용자 요청) — 뺐다. */}
      <Box
        sx={{
