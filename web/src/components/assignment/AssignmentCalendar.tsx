 ...
 *
 *   A(project 범례, ~10%) | B(달력 그리드, ~60%) | C(선택한 날의 event 목록, ~30%)
 *
    ...
    <>
      <Box
        sx={{
          ...
          gridTemplateColumns: 'minmax(128px, 1fr) minmax(360px, 6fr) minmax(230px, 3fr)',
  ...
  return (
    ...
    >
      <Box sx={{ padding: '13px 15px', borderBottom: `1px solid ${T.ln}`, background: T.sf2 }}>
        <Box sx={{ fontSize: 13, fontWeight: 700 }}>{formatDayLabel(key)}</Box>
  ...
  return (
    ...
    >
      <Box sx={{ display: 'flex', alignItems: 'center', gap: '7px', flexWrap: 'wrap' }}>
        ...
        <Box sx={{ fontFamily: FONT_MONO, fontSize: 12, fontWeight: 700 }}>{row.label}</Box>
        <Box sx={{ fontSize: 12.5, fontWeight: 700, minWidth: 0, overflowWrap: 'anywhere' }}>
          {row.workflowAt.name}
        </Box>
        <Box sx={{ fontSize: 11, color: T.dm }}>{canonicalDepartmentLabel(row.workflowAt.department)}</Box>
        <Box sx={{ flex: 1 }} />
      ...
      <Box sx={{ fontSize: 11, color: T.dm, mt: '4px' }}>
        ...
        {row.published ? ' · published by my department' : ''}
      </Box>
    </Box>
  );
}
  ...
  return (
    ...
    >
      <Box sx={{ display: 'flex', alignItems: 'center', gap: '7px', flexWrap: 'wrap' }}>
        ...
        >
          {event.isPublished ? 'VERSION' : 'WORKING'}
        </Badge>
        <Box sx={{ fontSize: 12.5, fontWeight: 700, minWidth: 0, overflowWrap: 'anywhere' }}>
          {event.artifactName}
        </Box>
        <Box sx={{ fontFamily: FONT_MONO, fontSize: 12, color: T.pr }}>{event.versionLabel}</Box>
        <NetworkTag network={event.network} />
        <Box sx={{ flex: 1 }} />
        <Box sx={{ fontSize: 10.5, color: T.dm2, fontFamily: FONT_MONO }}>{fmtAt(event.occurredAt)}</Box>
      </Box>
      <Box sx={{ fontSize: 11, color: T.dm, mt: '4px' }}>
