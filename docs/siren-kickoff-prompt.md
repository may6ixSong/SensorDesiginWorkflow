...
> 프로젝트 지식(Project Knowledge)에는 `siren-design-v2.md`와 `analog-dashboard-v15.html`을 함께 올려두세요.
...
- MongoDB 연결 문자열 (예: mongodb://localhost:27017/siren):
...
너는 지금부터 **SIREN**(아날로그 산출물 릴리스 플랫폼)의 FE/BE 소스코드를 처음부터 구축한다.

### 참조 자료
- **`siren-design-v2.md`** — 데이터 모델, API 계약, 권한 정책, 도메인 규칙의 정본. 여기 명시된 규칙(특히 3장 도메인 규칙, 6장 권한 재검증)은 임의로 바꾸지 말고 그대로 구현할 것.
