...
### 4.3 부서별 release item 상태/코멘트

release 안 산출물 하나하나에 대해, 그걸 받은 **한 부서**가 상태와 코멘트를 남길 수 있다
(사용자 확정 — "추후 workflow에서 부서별로 release status를 보기 위함"). 상태는 세 가지뿐이다.
...
- 새 컬렉션 `ReleaseItemFeedback`을 둔다 — §6의 "새 컬렉션을 두지 않는다" 원칙은 **기존
  데이터를 부서 관점으로 다시 베끼는 것**을 금지한 것이고, 이건 그런 사본이 아니라 그 자체로
  원본인 새 사실(누가 언제 어떤 상태/코멘트를 남겼는가)이라 해당하지 않는다.
- comment와 status는 한 쌍이고, **append-only**다 — Release 자체의 철회 불가 원칙(05장
  §4.5)과 같은 이유로 update/delete 라우트를 두지 않는다. "현재 상태"는 가장 최근 항목이다.
- `GET/POST /releases/:releaseId/items/:blockId/feedback` (department는 쿼리/바디로 필수).
  판정 순서: ① 그 release를 볼 자격(§4.1) → ② 그 department가 실제로 그 item의
  recipient인가(아니면 400) → ③ actor가 지금 이 과제에서 그 department 소속인가, 또는
  Admin(아니면 403).
- **다른 부서에는 이 이력 자체가 보이지 않는다** — 조회가 항상 department 하나로 좁혀지고,
  다른 department 파라미터로는 위 ②/③에서 걸러진다. "전체 부서" 조회는 없다.
