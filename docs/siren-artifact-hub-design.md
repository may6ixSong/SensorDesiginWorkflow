...
### 11.5 Calypso 인터페이스 — BE는 분리, FE는 당분간 SIREN 안에서 (결정 변경)

§3.3은 "FE도 SIREN과 별개 화면/경로로 둔다"고 적었지만, 실제로 화면을 만들어 보고 그 판단을
**뒤집었다**. 바뀐 건 인터페이스의 위치뿐이고, 데이터 소유권은 그대로다.

| 항목 | 결정 | 이유 |
|---|---|---|
| **Calypso BE** | 계속 분리 (system of record) | 산출물의 실물·버전 이력은 Calypso DB가 소유한다. SIREN DB에 파일을 쌓기 시작하면 나중에 떼어낼 때 이관 대상이 데이터가 되고, 그 시점은 실무적으로 오지 않는다 |
| **Calypso FE (`calypso/web`)** | **삭제됨** (당분간 쓰지 않음 → 실제로 제거, 사용자 요청) | 사용자 입장에선 SIREN이든 Calypso든 화면만 같으면 차이가 없는데, 앱을 분리하면 "업로드하러 딴 서비스로 가라"는 이동 비용만 남는다. 쓰지 않는 채로 남겨두는 것보다 걷어내는 쪽을 택했다 |
| **당분간의 등록 창구** | **SIREN web 안의 페이지**가 Calypso api를 직접 호출 | project 메타·SSO·권한이 전부 SIREN 것과 동일하므로 앱을 갈라서 얻는 게 없다 |

**되돌리는 조건.** 다음 두 가지가 같이 성립하면 그때 인터페이스를 다시 `calypso/web`(또는 그 자리의
새 앱)으로 옮긴다.

1. 산출물이 실제로 많이 쌓여 Calypso 자체 화면(검색·필터·대량 관리)이 필요해질 때
2. 산출물이 "파일 업로드"가 아니라 **template 기반의 구조화된 데이터 저장**으로 바뀔 때 —
   그 시점엔 Calypso가 자기만의 편집 UI를 갖게 되므로 SIREN 안에 복제할 이유가 사라진다

BE가 이미 분리돼 있어 데이터 이관은 여전히 없지만, `calypso/web`을 실제로 지웠으므로 그때는 화면
코드를 **git 이력에서 복원하거나 새로 작성**해야 한다 — "지우지 않고 두면 전환 비용이 0에 가깝다"는
당초 전제는 더 이상 유효하지 않다.

**대가로 감수하는 것.** SIREN 프론트엔드가 Calypso api의 모양을 직접 알게 된다 — "SIREN은
등록된 서비스가 무엇인지 몰라도 된다"는 §3.2의 원칙에서 이 한 서비스만 예외가 된다. Hub 서비스가
둘 이상 이 방식을 요구하기 시작하면 그때는 예외가 아니라 구조 문제이므로, `embedUploadUrlTemplate`
기반의 임베드(§3.2)로 일반화해야 한다.
...
### 15.4 데이터 소스 (2026-09-03 결정 번복)

**API를 부르지 않는다.** 처음엔 `artifactServices` 레지스트리 조회로 교체해 대문이 실제
Hub 상태를 반영하게 했었는데, 실제로 붙여보니 배포 환경마다(레지스트리 시드 여부, api
가용성, CORS 등) 대문이 빈 판넬로 보이는 문제가 생겼다 — **대문은 그냥 대문이라 데이터가
실제 상태를 반영할 필요가 없다**는 게 최종 판단이다.

`web/src/config/hubShowcase.ts`에 4개 슬랩과 각각의 대표 산출물을 고정 배열로 둔다.
API 호출 없이 이 파일만으로 렌더되고, 값을 바꾸고 싶으면 이 파일만 고치면 된다 — 다른
Hub 화면(§11.4 산출물 선택, §13.4 레지스트리 관리)은 여전히 실제 `/hub/services`를
부른다. 이 원칙 번복은 대문에만 적용된다.
...
POC 성격이라 기존 `siren` DB를 직접 고치지 않는다 — **새 DB에 복사본을 만든다.**

> **작업 범위: 지금 `siren` DB가 쓰고 있는 컬렉션 전체(`projects`, `workflows`, `deliverables`,
> `memos`, `edges`, `hldReleases`, `auditLogs`)를 새 DB로 복사한다.** `deliverables`만
> `docs/siren-artifact-hub-design.md` §10.4 규칙대로 바꿔서 옮기고(실제 등록된 산출물이 없는
> 유휴 상태이므로, 기존 `versions[]`를 정교하게 변환하는 게 아니라 §11의 "정상 빈 상태"로 리셋
> — `docType`/`artifactKey` 삭제, `serviceKey`/`externalArtifactId`는 `null`), 나머지는
> 그대로 복사한다. 새 스키마가 쓰는 신규 컬렉션(`artifactServices`, `hubSyncCheckpoints`, §10.3)도
> 빈 컬렉션으로 새 DB에 만들어둔다(내용을 채우진 않는다 — 그건 이 작업의 범위가 아니다).
> **원본 `siren` DB는 절대 건드리지 않는다** — 읽기만 한다.
>
> **절대 규칙 — 하나라도 어기면 작업을 멈추고 보고할 것:**
>
> 1. **먼저 새 git 브랜치를 만들어 체크아웃한 뒤 시작한다.** `main`이나 이 설계서가 올라간
>    브랜치에서 직접 작업하지 않는다 — POC라 격리된 브랜치 위에서 진행한다. (아래 규칙 5 때문에
>    이 브랜치에 실제로 커밋이 쌓이진 않겠지만, 안전장치로 둔다.)
> 2. **애플리케이션 코드는 단 한 줄도 건드리지 않는다.** `api/src/**`, `web/src/**`의 어떤 파일도
>    수정·생성·삭제하지 않는다. Mongoose 스키마 파일(`deliverable.schema.ts` 등)도 포함이다 —
>    코드는 이번 작업 범위가 아니고, 별도의 후속 작업에서 다룬다.
> 3. **DB 접속은 앱의 Mongoose 모델을 거치지 않는다.** 원시 MongoDB 드라이버(또는 `mongosh`)로
>    소스(`siren`)와 타겟(새 DB) 양쪽에 직접 접속해서 읽고 쓴다. 타겟 DB 이름은 아직 정해지지
>    않았다 — 이 시점에 사용자에게 확인하거나, 없으면 `siren_hub_poc` 정도의 이름으로 새로
>    만든다.
> 4. **변환에 스크립트가 필요하면 임시 파일로 작성해서 실행한 뒤, 작업이 끝나면 그 스크립트
>    파일을 반드시 삭제한다.** 리포지터리에 마이그레이션 스크립트를 남기지 않는다.
> 5. **`git add`, `git commit`, `git push`를 포함해 어떤 git 쓰기 명령도 실행하지 않는다.**
>    이건 특히 중요하다 — **절대 커밋하지 않는다.** 이 작업은 새 데이터베이스에 문서를 새로
>    쓰는 것이지, 리포지터리에 남는 변경사항이 아니어야 한다(스크립트 자체도 규칙 4에 따라
>    지워지므로 커밋할 대상이 애초에 남지 않는다). 규칙 1의 브랜치는 격리 목적일 뿐, 거기에도
>    커밋하지 않는다.
> 6. 작업 전후로 컬렉션별 원본 문서 수와 새 DB에 쓰인 문서 수를 대조해 보고한다(예: "projects
>    N개, workflows N개, deliverables N개(§10.4 규칙 적용) ... 원본과 전부 일치"). 실패한
>    문서가 있으면 어떤 컬렉션의 어떤 문서인지, 왜 실패했는지 구체적으로 보고하고 나머지는
>    계속 진행한다.
>
