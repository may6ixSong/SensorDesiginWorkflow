...
| **Calypso FE (`calypso/web`)** | **당분간 쓰지 않음 (deprecated)** | 사용자 입장에선 SIREN이든 Calypso든 화면만 같으면 차이가 없는데, 앱을 분리하면 "업로드하러 딴 서비스로 가라"는 이동 비용만 남는다 |
| **당분간의 등록 창구** | **SIREN web 안의 페이지**가 Calypso api를 직접 호출 | project 메타·SSO·권한이 전부 SIREN 것과 동일하므로 앱을 갈라서 얻는 게 없다 |

**되돌리는 조건.** 다음 두 가지가 같이 성립하면 그때 인터페이스를 `calypso/web`으로 옮긴다.
...
그때의 전환 비용은 낮다. BE가 이미 분리돼 있어 데이터 이관이 없고, SIREN 쪽에서 걷어낼 것은
화면 코드와 진입점 링크뿐이다. `calypso/web`은 지우지 않고 그대로 둔다 — 설계 결정이 최종적으로
확정된 게 아니라 시점을 미룬 것이기 때문이다.
