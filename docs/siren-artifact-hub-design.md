...
2. SIREN 레지스트리에 항목을 추가한다(관리자 화면 또는 seed).
...
1. **실제 호출자는 검증된 자격증명에서만 얻는다.** SSO 연동 후에는 서명된 토큰에서 뽑는다. 클라이언트가 보낸 평문 필드를 실제 신원으로 신뢰하지 않는다.
...
3. **Admin 목록은 서버 설정에 둔다.** DB의 `isAdmin` 플래그 하나에 의존하지 않는다.
...
1. **File Vault 레포명과 배포 위치** — 사내 어느 인프라에 올릴지.
2. **HPC 공용 DB의 실제 엔진·스키마 권한** — 수출 테이블 생성 권한을 누가 갖는지.
3. **Admin 목록의 저장 위치** — 환경변수 / 설정 파일 / 별도 컬렉션.
4. **`sourceRefs` 수집의 실효성** — 기존 시스템이 자기 입력 버전을 실제로 추적하고 있는지는 시스템마다 다르다. 못 주는 곳은 빈 배열로 두고, 이 필드를 필수로 만들지 여부는 4단계 이후 결정한다.
5. **C/D 산출물의 Release 참여 여부** — 신뢰도가 낮은 항목을 Workflow Release에 포함시킬지, 아니면 경고 후 포함할지.
6. **레지스트리 관리 화면** — 서비스 등록을 UI로 할지 seed 스크립트로만 할지.
