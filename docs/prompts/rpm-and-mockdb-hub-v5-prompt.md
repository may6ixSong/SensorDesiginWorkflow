...
아래 프롬프트를 그대로 데스크톱 세션에 붙여넣으면 된다. 붙여넣기 전에 채워 넣을 것:
- **RPM 로컬 저장소 경로** (`<RPM_REPO_PATH>` 표시된 자리)
...
3. `artifacts` 컬렉션에서 tier가 `A`/`B`/`C`인 문서 중 `editAccess`/`viewAccess`가 비어있지
   ...
   `artifactServices`의 레거시 `viewUrlTemplate`/`sampleUrl` 하위 필드도 원한다면 같이 정리한다.
...
4. 아래 문서를 **DB에 쓰기 전에 내용을 사용자에게 보여주고 진행해도 되는지 확인받은 뒤** 삽입한다:
   ...
   ```
5. **삽입에 쓴 스크립트 파일은 실행 직후 바로 삭제한다** (원칙 3과 동일하게).
