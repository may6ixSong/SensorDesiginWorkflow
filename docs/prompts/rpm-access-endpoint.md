...
## 0. 작업 범위 — 어느 저장소를 고치는가

**고치는 곳은 이 서비스(RPM 등)의 저장소뿐이다.** 여기서는 코드를 자유롭게 추가·수정하고
커밋해도 된다 — 애초에 그게 이 요청의 목적이다.

**SIREN 저장소는 건드리지 않는다.** 만약 같은 작업 공간에 SIREN이 함께 열려 있더라도,
그쪽 파일을 고치거나 `git add` / `git commit` 하지 않는다. SIREN 쪽은 이미 이 계약에 맞게
구현이 끝나 있고, 추가로 수정할 것이 없다.
