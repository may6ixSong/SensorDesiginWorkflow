...
| T13 | "새 Artifact 추가" 다이얼로그 단일 목록 재구성 | 지금은 OA Service/File Artifacts/HPC Service 3버튼으로 source를 먼저 고르는 방식. 목표는 admin이 등록한 OA/HPC Service와 Calypso 산출물을 한데 합친 단일 목록(04장 §6) — 이번 변경(Tier D 폐기·File Artifacts 확장)에서는 후보 API(`ArtifactSourceService`)에 TODO만 남기고 UI는 손대지 않았다 |
| T14 | File Artifacts(Calypso) 받는 쪽 placeholder의 편집권 | 받는 쪽이 매핑용으로 새로 등록한 artifact도 지금은 등록자가 자동으로 편집 가능하다(Calypso `computeAccess()`의 기존 규칙을 그대로 둠, 사용자 결정) — 원래는 편집 불가로 가기로 했던 것을 나중에 구체화한다 |
| T15 | File Artifacts(Calypso) 다중 파일 다운로드/업로드 UI | 스키마·API는 여러 파일을 지원하도록 바꿨지만(한 버전에 파일 여러 개, `POST .../versions`의 `files` 필드), FE는 아직 첫 번째 파일만 업로드/다운로드한다 — `web/src/api/calypsoClient.ts`, `ArtifactVersionContents.tsx`, `CalypsoInlinePanel.tsx`에 TODO로 남겨뒀다 |
| T16 | File Artifacts(Calypso) OA-link/HPC-path 등록 UI | 백엔드(스키마·서비스·컨트롤러)는 `network`에 따라 File(복수 파일)/OA-link(`viewUrl`)/HPC-path(`hpcPath`) 콘텐츠를 받을 수 있지만, "새 Artifact 추가" 다이얼로그에는 아직 OA-link/HPC-path로 등록하는 화면이 없다 — File 등록 흐름만 살아있다 |
