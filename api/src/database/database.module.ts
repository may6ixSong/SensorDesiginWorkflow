...
import { HpcPathMockSchema } from '../artifacts/schemas/hpc-path-mock.schema';
...
const ALL_MODELS: ModelDef[] = [
  ...
  { name: 'HpcPathMock', schema: HpcPathMockSchema },
...
/**
 * DB_CONNECTION(AES 암호문) + AES_KEY가 설정되어 있으면 그 실제 DB에 연결한다.
...
export class DatabaseModule {
  constructor() {
    if (!isUsingRealDb()) {
      logger.warn(
        'DB_CONNECTION/AES_KEY가 설정되지 않아 실제 DB에 연결하지 않습니다. 순수 인메모리 목업 데이터로 동작합니다 ' +
          '(프로세스를 재시작하면 초기화됩니다). 지속되는 데이터가 필요하면 api/.env.development의 ' +
          'DB_CONNECTION(AES 암호문)과 AES_KEY를 설정하세요.',
      );
    }
  }
}
