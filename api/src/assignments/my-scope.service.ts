...
export interface MyScope {
  ...
  myOwnNodes: ScopedNode[];
  /** myOwnNodes가 가리키는 artifact id 집합(중복 제거). */
  myArtifactIds: string[];
}

/**
 * scope 해석은 My Assignment의 모든 엔드포인트가 맨 앞에서 한 번씩 부른다.
 * 쿼리는 최대 3방이다 — projects → workflows → nodes. 그 뒤로는 전부 이 결과 위에서
...
export class MyScopeService {
  ...
  async resolve(actor: Actor): Promise<MyScope> {
    ...
    const myOwnNodes = myWorkflows.length ? await this.ownNodesOf(myWorkflows) : [];
    const myArtifactIds = [...new Set(myOwnNodes.map((n) => n.artifactId))];
  ...
  /**
   * 그 workflow들의 node 중 **내가 주는 산출물이면서 실제 artifact가 매핑된 것**만.
   * 미매핑 node(`artifactId: null`)은 전달할 실체가 없으므로 애초에 대상이 아니다
   * (설계서 05장 §2.2와 같은 이유).
   */
  private async ownNodesOf(workflows: ScopedWorkflow[]): Promise<ScopedNode[]> {
